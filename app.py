...
def yahoo_symbol(ticker):
    """Convert common Indian NSE tickers to Yahoo Finance symbols."""
...
def fetch_prices(tickers):
    """
    Fetch current/latest prices directly from Yahoo's chart endpoint.
    This avoids relying on yfinance's fast_info, which can fail on
    Streamlit Cloud even when Yahoo data itself is available.
    """
    if not tickers:
        return {}
    ...
    for original in tickers:
        symbol = yahoo_symbol(original)
        try:
            url = (
                "https://query1.finance.yahoo.com/v8/finance/chart/"
                f"{symbol}?range=1d&interval=1m&includePrePost=false"
            )
            ...
            # Best available current/regular-market price.
            price = (
            ...
            # Fallback to the latest chart close.
            if price is None:
            ...
            if price is not None:
                out[original] = float(price)

            # Avoid hammering Yahoo when several holdings exist.
            time.sleep(0.15)
...
with st.sidebar:
    ...
    if st.button("↻ Refresh Prices", use_container_width=True, disabled=not is_admin()):
        ...
        # Force a fresh Yahoo Finance request instead of reusing the 60-second cache.
        fetch_prices.clear()
        ...
        if n:
            ...
            st.error(
                "No market prices were returned. Check the Yahoo/NSE ticker symbols "
