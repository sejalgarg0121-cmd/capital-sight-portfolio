...
def yahoo_symbol(ticker):
    ...
    if not ticker:
        return ticker
    # Yahoo Finance uses .NS for NSE-listed Indian equities.
    # Keep indices (^NSEI etc.) and already-qualified symbols unchanged.
...
def fetch_prices(tickers):
    ...
    for original in tickers:
        ...
        try:
            # history() is more reliable here than relying only on fast_info.
            ...
            if hist is not None and not hist.empty:
                ...
                if not valid.empty:
                    out[original] = float(valid.iloc[-1])
                    continue

            # Fallback when 1-minute data is unavailable.
            ...
            # One bad ticker must not stop the entire portfolio refresh.
            continue
